...
* добавлен выбор параллельного перевода в toolbar;
* параллельная глава загружается через существующий chapter endpoint;
* второй текст показывается под соответствующим стихом;
* выбранный параллельный перевод сохраняется в reader localStorage state.

Результат:

* reader получил первый рабочий режим нескольких переводов без нового backend API.
