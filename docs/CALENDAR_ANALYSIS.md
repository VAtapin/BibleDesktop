...
* `type=10` по найденным записям соответствует постам и постным периодам.

Примеры `type=10`:

```text
Великий пост: s_month=0, s_date=-48, f_month=0, f_date=-1
Петров (Апостольский) пост: s_month=0, s_date=57, f_month=6, f_date=28
Успенский пост: s_month=8, s_date=1, f_month=8, f_date=14
Рождественский (Филиппов) пост: s_month=11, s_date=15, f_month=12, f_date=24
```
...
* отдавать постные события отдельно как `fasting_events`.
...
* Где источник Евангелия и Апостола дня? В `OLD/bible-desktop.sql` отдельная таблица чтений не обнаружена.
* Где источник постных правил? Базовые постные периоды есть в `MemoryDays.xml` как `type=10`, но степень поста и подробный устав не найдены.
