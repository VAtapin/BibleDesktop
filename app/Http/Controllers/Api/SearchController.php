...
class SearchController extends Controller
{
    public function verses(Request $request): JsonResponse
    {
        ...
        $results = DB::table('verse_texts')
            ...
            ->map(fn ($row) => [
                ...
                'text' => $row->text,
                'snippet' => $this->snippet((string) $row->text_plain, $query),
            ]);
     ...
     */
    private function referenceResults(array $reference, string $translationCode, int $limit): \Illuminate\Support\Collection
    {
        return DB::table('verse_texts')
            ...
            ->map(fn ($row) => [
                ...
                'verse_number' => $row->verse_number,
                'text' => $row->text,
                'snippet' => mb_substr((string) $row->text_plain, 0, 160),
            ]);
    }
