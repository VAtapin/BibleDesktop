...
* Добавлена таблица `legacy_canonical_chapter_overrides`; `bible:legacy:import-metadata` уже поддерживает безопасное действие `map_chapter`, а `bible:legacy:report-skipped-verses` классифицирует override actions отдельно от обычных mapping misses.
...
1. Наполнить canonical mapping overrides реальными правилами и научить report классифицировать heading/appendix/non-canonical cases.
