...
Версия плана: 0.3  
Дата: 2026-06-19  
Статус: создан Laravel/Vue/Filament foundation, базовый канон, API справочников/главы, admin resources, legacy metadata importer, первый импорт RST стихов, Strong-токенов и cross references, православный календарь и ручная модель чтений дня
...
* подготовить Docker окружение - начато, добавлен compose для Laravel app на PHP 8.4, queue worker, PostgreSQL+pgvector и Redis;
...
* проверить Docker import на локальном PostgreSQL - выполнено для `L1_RST`, `L2_MDR`, Strong, cross references и calendar events.
...
php artisan bible:legacy:import-supplemental-texts
...
php artisan calendar:import-readings --path=storage/app/calendar-readings.csv
...
* API supplemental legacy texts - начато: `GET /api/translations/{translation}/supplemental-texts`;
...
* режим нескольких переводов - начато: второй перевод загружается через chapter endpoint и показывается под основным стихом;
* контекстное меню стиха - начато: копирование ссылки/текста и открытие справочника;
...
* чтения дня - начата ручная/import-ready таблица `calendar_readings`;
* Евангелие дня - Telegram/API читают `calendar_readings`, источник наполнения ещё нужен;
* Апостол дня - Telegram/API читают `calendar_readings`, источник наполнения ещё нужен;
* API календаря - начато: `GET /api/calendar/day`;
...
* `/gospel` - начато, читает `calendar_readings` и возвращает fallback, если чтение не задано;
* `/apostle` - начато, читает `calendar_readings` и возвращает fallback, если чтение не задано;
...
* личные заметки - начато: demo-user API и reader form для заметок к стиху;
* публичные заметки;
* групповые заметки;
* теги;
...
* Docker infrastructure стартует с Laravel app container, queue worker, PostgreSQL 16 + pgvector и Redis; Windows helper-скрипты остаются локальным dev-путём.
* Docker runtime обновлён до PHP 8.4, потому что текущий `composer.lock` требует Symfony packages с PHP >= 8.4.1; app/queue image также включает `redis` extension для Laravel Redis queue/cache.
* Docker compose монтирует локальную папку `OLD/` read-only для импортёров; `OLD/` остаётся вне Git и вне Docker image.
* Локальный Docker PostgreSQL стенд наполнен минимальным рабочим набором: `L1_RST` 37050 стихов, `L2_MDR` 30874 стихов, 14696 Strong entries, 458984 Strong tokens, 540781 cross references, 3811 calendar events.
...
* После повторного `bible:legacy:import-metadata` mapped Baruch 6 перестаёт быть skipped, а повторный `bible:legacy:import-verses --library=10` импортирует DRB без skipped и создаёт `L10_DRB EpJer.1.1`; до verse-level Joel rules оставались 196 classified skipped rows для appendix/heading/requires_* cases.
* `bible:legacy:import-supplemental-texts` переносит heading/appendix/non-canonical legacy rows в отдельную таблицу `legacy_supplemental_texts`; на `database/testing.sqlite` импортировано 92 строки: 65 appendix и 27 heading.
* Supplemental legacy texts доступны через `GET /api/translations/{translation}/supplemental-texts` с фильтрами `book`, `type`, `limit`; route-list теперь показывает 12 API routes.
* Для чтений дня добавлена таблица `calendar_readings`: поддерживаются fixed и pascha-relative правила, типы `gospel`/`apostle` и ссылка на отрывок.
* `GET /api/calendar/day` теперь возвращает `readings`, а Telegram `/gospel` и `/apostle` читают эти данные вместо жёсткой заглушки.
* Добавлена команда `calendar:import-readings` для UTF-8 CSV с колонками `date_rule_type,month,day,offset,reading_type,title,passage_ref,sort_order`; это import-ready слой для внешнего или ручного источника Евангелия/Апостола дня.
* Добавлена таблица `legacy_canonical_verse_overrides`; `bible:legacy:seed-canonical-overrides` создаёт 104 verse-level правила для 4-главного Joel: `Joel 3:1-5 -> Joel 2:28-32`, `Joel 4:1-21 -> Joel 3:1-21`.
* `bible:legacy:import-verses` применяет `map_verse` до chapter mapping, поэтому может импортировать стихи из legacy-глав без `canonical_chapter_id`; `bible:legacy:report-skipped-verses` не считает такие стихи skipped.
* После seeding verse overrides skipped report на `database/testing.sqlite` снизился до 112 строк: 65 appendix, 27 heading, 20 конфликтных `requires_book_mapping`.
* Переимпорт `L3_UKR`, `L5_LB`, `L325_UKH`, `L359_SCH2000NEU` подтвердил Joel mapping: legacy `Joel 3:1` уходит в `Joel.2.28`, legacy `Joel 4:1` уходит в `Joel.3.1`.
* `L3_UKR 2thessalonians 4 -> 1timothy 1` остаётся отдельной конфликтной задачей, потому что в той же библиотеке уже есть нормальная `1timothy 1`, и прямой auto-map перезапишет корректный текст.
* Конфликтный `L3_UKR 2thessalonians 4` сохраняется через `bible:legacy:import-supplemental-texts` как `type=requires_book_mapping`, не попадая в canonical `verse_texts` и не перезаписывая `1Tim 1`.
* Search API получил PostgreSQL Full Text Search path через `to_tsvector/plainto_tsquery/ts_rank` и GIN index migration; SQLite остаётся fallback через `LIKE`.
* Search API возвращает `snippet_segments`, а reader UI подсвечивает совпадения безопасно без `v-html`.
* Docker compose получил `app` service на Laravel `artisan serve` и `queue` service на `queue:work`; `Dockerfile` собирает Composer dependencies и Vite assets.
* Reader UI получил контекстное меню стиха по правому клику: копировать ссылку, копировать текст, открыть справочник; сообщение действия исчезает автоматически.
* Reader UI получил начальный режим нескольких переводов: дополнительный select в toolbar загружает параллельную главу и показывает второй текст под соответствующим стихом.
* Notes MVP добавляет `GET/POST /api/verses/{verse}/notes` с demo-user fallback до полноценной авторизации; reader panel показывает и создаёт заметки выбранного стиха.
