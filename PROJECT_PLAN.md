...
Версия плана: 0.3  
Дата: 2026-06-19  
Статус: создан Laravel/Vue/Filament foundation, базовый канон, API справочников, legacy metadata importer и первый импорт RST стихов
...
* импортировать библиотеки - выполнено для supported Bible modules через `bible:legacy:import-metadata`;
...
* Первичная legacy metadata миграция ограничена поддерживаемыми языками проекта: `ru`, `uk`, `be`, `de`, `en`.
...
* Strong-маркеры в `verse.vers` сохраняются в `verse_texts.text_raw`, но очищаются из display-поля `text`; отдельная нормализация Strong tokens будет следующим шагом.
...
1. Подготовить API чтения главы на `verse_texts`.
2. Подготовить импорт Strong tokens из очищенной RST-разметки.
3. Подготовить импорт `quote` / cross references.
4. Расширить Filament для переводов, модулей и legacy mapping.
5. Подготовить reader flow на реальных данных.
