...
* выбор книги - начато в reader shell;
...
* `/today` - placeholder до импорта календаря;
* `/gospel` - placeholder до импорта календаря;
* `/apostle` - placeholder до импорта календаря;
* `/random` - skeleton готов, отправка через Bot API подготовлена;
* `/search` - начато, использует базовый поиск по `verse_texts`;
* `/calendar` - placeholder до импорта календаря;
* `/fasting` - placeholder до импорта календаря;
...
* Telegram Bot `/search` использует текущий DB-agnostic поиск по `verse_texts`; календарные команды возвращают честный placeholder до импорта календаря.
...
* Search API добавляет базовый DB-agnostic поиск по `verse_texts.text_plain`; PostgreSQL Full Text Search остаётся следующим улучшением.
