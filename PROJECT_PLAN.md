...
* Добавлена таблица `legacy_canonical_chapter_overrides`; `bible:legacy:import-metadata` уже поддерживает безопасное действие `map_chapter` для legacy chapters, у которых нет обычного canonical lookup.
