...
Первая вкладка reader — лёгкая главная панель с карточками разделов и счётчиками. При старте запрашивается только компактный `GET /api/home`; справочники переводов, книги, главы, рецепты, тесты и остальные разделы загружаются асинхронно по необходимости. Прямые ссылки на конкретную главу сохраняют ожидаемое поведение и сразу открывают текст.
...
* Социальная лента: публикации к стихам, открытие связанного стиха по клику, фильтр по текущей книге, базовые таблицы подписок `user_follows` и дружбы `user_friendships`.
...
* `GET /api/home` — кэшируемые счётчики публичных разделов для первой загрузки;
