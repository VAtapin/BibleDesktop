...
Длинные молитвы редактируются в `/admin/prayers`: основное поле `Текст молитвы` можно использовать для короткой версии или вступления, а блок `Части / страницы молитвы` хранит страницы в `prayer_sections`. На сайте список молитв остается компактным, выбранная молитва открывается в основном окне, а части показываются отдельными переключателями и подгружаются по мере выбора.
